...
env.git_code = "ssh://git@lab.admicro.vn:2317/toannh/adx2016.git"
	...
	try {
		...
		stage('Link_config_file') {
			...
			if (fileExists("$web_shared")) {
				...
				sh 'cd $web_release/$(tail -1 $release_note) && rm -rf public && ln -s ../../shared/public'
				sh 'cd $web_release/$(tail -1 $release_note)/glb && rm -rf cfglb.php && ln -s ../../../shared/glb/cfglb.php'
				sh 'cd $web_release/$(tail -1 $release_note)/ecom && rm -rf public && ln -s ../../../shared/ecom/public'
				sh 'cd $web_release/$(tail -1 $release_note)/mobile && rm -rf public && ln -s ../../../shared/mobile/public'
			}
			else {
